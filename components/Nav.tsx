...
export function Nav() {
  ...
  return (
    <>
      ...
      {/* Outside header: backdrop-filter on sticky header makes fixed descendants use the header as their containing block (overlay collapses after scroll). */}
      <div
        id="mobile-nav"
        className={`fixed inset-0 top-[3.75rem] z-40 bg-nav-menu transition-[opacity,visibility] duration-300 md:hidden ${
        ...
        <ul className="relative z-10 flex flex-col px-6 py-8">
          {navLinks.map((link) => (
            <li key={link.href}>
              <Link
                href={link.href}
                className={`block border-b border-border ${navLinkClassName}`}
                onClick={() => setMenuOpen(false)}
              >
                {link.label}
              </Link>
